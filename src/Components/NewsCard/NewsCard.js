...
const NewsCard = ({ article: { description, publishedAt, source, title, url, urlToImage }, i, activeArticle, length }) => {
    ...
    useEffect(() => {
        ...
        setElRefs((refs) => Array(length).fill().map((_, j) => refs[j] || createRef()));
    }, []);
